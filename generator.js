...
class RelatorioPDF {
  constructor() {
    ...
    if (!logStats.error) {
      log(
        `📊 Logs: ${(logStats.currentLogSize / 1024).toFixed(2)}KB atual, ${
          logStats.archiveCount
        } arquivados`,
        "info"
      );
    }
  ...
  logConfigDetails(config) {
    ...
    if (config.relatorio) {
      log(
        `  📅 Período: ${config.relatorio.periodo || "Não definido"}`,
        "data"
      );
      log(
        `  📆 Data Geração: ${config.relatorio.dataGeracao || "Não definida"}`,
        "data"
      );
      log(
        `  🏷️ Plataforma: ${config.relatorio.plataforma || "Não definida"}`,
        "data"
      );
    }
    ...
    if (config.configuracoes) {
      log(
        `  💰 Investimento mensal: ${
          config.configuracoes.investimentoMensalMedio || "Não definido"
        }`,
        "data"
      );
      log(
        `  🎯 Meta ROAS: ${config.configuracoes.metaROAS || "Não definida"}`,
        "data"
      );
    }
  }
  ...
  loadTemplate() {
    ...
    try {
      ...
      log(
        `✅ Template HTML carregado com sucesso (${templateSize} KB)`,
        "success"
      );
  ...
  try {
    ...
    for (const imagePath of possiblePaths) {
      if (fs.existsSync(imagePath)) {
        ...
        console.log(`📸 Usando imagem: ${imagePath}`);
    ...
    if (!imageFound) {
      ...
      console.log('🔍 Caminhos verificados para imagens:');
    ...
    const patterns = [
      // Padrões para a nova imagem 1.1.jpg
      /src="[^"]*1\.1\.jpg"/g,
      /src='[^']*1\.1\.jpg'/g,
      
      // Padrões para logo antigo (compatibilidade)
      /src="[^"]*logo-que-vai-aprovar\.(webp|svg|png|jpg)"/g,
      /src='[^']*logo-que-vai-aprovar\.(webp|svg|png|jpg)'/g,
      
      // Padrões gerais para pasta img
      /src=["'][^"']*\/img\/[^"']*\.(jpg|webp|svg|png)["']/g,
      /src=["']\.\/img\/[^"']*\.(jpg|webp|svg|png)["']/g,
      /src=["']\.\.\/img\/[^"']*\.(jpg|webp|svg|png)["']/g
    ];
    ...
    patterns.forEach((pattern, index) => {
      ...
      if (matches) {
        ...
        log(`✓ Padrão ${index + 1}: ${matches.length} referências substituídas`, 'info');
        console.log(`✓ Padrão ${index + 1}: ${matches.length} referências → Base64`);
      }
    });

    if (replacements > 0) {
      log(`✅ ${replacements} referências de imagem convertidas para Base64`, 'success');
      console.log(`✅ Total: ${replacements} referências convertidas para Base64`);
    } else {
      log('⚠️ Nenhuma referência de imagem encontrada no HTML', 'warning');
      console.log('⚠️ Nenhuma referência de imagem encontrada no HTML para substituir');
      
      // Debug: mostrar o que tem no HTML
      console.log('🔍 Verificando conteúdo do HTML...');
      const imgTags = html.match(/<img[^>]*>/g);
      if (imgTags) {
        console.log('📋 Tags <img> encontradas:');
...
convertImageToBase64(imagePath) {
  try {
    ...
    console.log(`✅ ${imagePath} → Base64 (${mimeType}, ${sizeKB} KB)`);
  ...
  processarDadosSimplificados(config) {
    ...
    console.log(
      `🔄 Analisando dados da plataforma: ${config.relatorio.plataforma}`
    );
    ...
    try {
      ...
      const metricas = this.calcularTodasMetricas(
        dadosOrganizados,
  ...
  organizarDadosPorPeriodo(dados) {
    ...
    dados.forEach((item) => {
      ...
      log(
        `  ✓ ${mes}/${ano}: ${item.pedidos} pedidos, ${this.formatarMoeda(
          item.valorBruto
        )}`,
        "info"
      );
  ...
  calcularTodasMetricas(dadosOrganizados, investimento, metaROAS) {
    ...
    // 4. EFICIÊNCIA
    metricas.eficiencia = this.calcularIndicadoresEficiencia(
      metricas.totais,
      meses.length
    );
  ...
  calcularMetricasMensal(dados2024, dados2025, investimento, metaROAS) {
    const crescPedidos = this.calcularCrescimento(
      dados2024.pedidos,
      dados2025.pedidos
    );
    const crescReceita = this.calcularCrescimento(
      dados2024.receita,
      dados2025.receita
    );
  ...
  calcularTotaisAno(dadosAno) {
    ...
    const totalPedidos = meses.reduce(
      (sum, mes) => sum + (mes.pedidos || 0),
      0
    );
    const totalReceita = meses.reduce(
      (sum, mes) => sum + (mes.receita || 0),
      0
    );
  ...
  calcularComparacoes(totais) {
    ...
    const crescTicket = this.calcularCrescimento(
      totais[2024].ticketMedio,
      totais[2025].ticketMedio
    );
  ...
  prepararDadosHTML(metricas, infoRelatorio) {
    ...
    const mesesComGraficos = this.calcularAlturasGraficosCompleto(
      metricas.porMes
    );
    ...
    const conclusao = {
      ...
      roasResultado: `${(
        metricas.totais[2025].receita / investimentoTotal
      ).toFixed(1)}x`,
    };

    // 4. DESTAQUE DINÂMICO
    const destaque = this.gerarDestaqueDinamico(
      infoRelatorio.plataforma,
      metricas.comparacoes,
      conclusao.roasResultado
    );
  ...
  calcularAlturasGraficosCompleto(porMes) {
    ...
    Object.keys(porMes).forEach((mesKey) => {
      ...
      log(
        `  ✓ Gráfico ${mesKey}: ${altura2024.toFixed(
          0
        )}% | ${altura2025.toFixed(0)}%`,
        "info"
      );
    });
  ...
  updateHTML(template, config) {
    ...
    try {
      ...
      const dadosCompletos = this.calcularDadosCompletosParaSecoes(
        dados,
        investimento
      );
      ...
      console.log(
        `  Total Pedidos 2025: ${dadosCompletos.formatados.pedidos2025}`
      );
      console.log(
        `  Total Receita 2025: ${dadosCompletos.formatados.receita2025}`
      );
  ...
  calcularDadosCompletosParaSecoes(dados, investimento) {
    ...
    dados.forEach((item) => {
      ...
      if (dadosPorMes[chave] !== undefined) {
        ...
        console.log(
          `  📅 ${chave}: ${item.pedidos} pedidos, ${this.formatarMoeda(
            item.valorBruto
          )}`
        );
      }
    ...
    meses.forEach((mes, index) => {
      ...
      // Ticket Médio
      const ticket2024 =
        dados2024.pedidos > 0 ? dados2024.receita / dados2024.pedidos : 0;
      const ticket2025 =
        dados2025.pedidos > 0 ? dados2025.receita / dados2025.pedidos : 0;
      const crescTicket = this.calcularCrescimento(ticket2024, ticket2025);
      const tendenciaTicket = this.definirTendencia(crescTicket);

      // ROI
      const roi = (
        ((dados2025.receita - investimento) / investimento) *
        100
      ).toFixed(0);
    ...
    // Métricas gerais
    const ticketMedio2024 =
      total2024.pedidos > 0 ? total2024.receita / total2024.pedidos : 0;
    const ticketMedio2025 =
      total2025.pedidos > 0 ? total2025.receita / total2025.pedidos : 0;
    ...
    return {
      ...
      formatados: {
        ...
        crescimentoTicket: this.calcularCrescimento(
          ticketMedio2024,
          ticketMedio2025
        ),
      },
  ...
  atualizarAnaliseTicketMedio(html, dados) {
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      ...
      if (html.match(linhaAntiga)) {
        ...
        console.log(
          `  ✅ Tabela ${mes}: ${metricas.ticketFormatado2025} (${metricas.crescTicket})`
        );
      }
    });
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      ...
      console.log(
        `  ✅ Gráfico ${mes}: 2024=${metricas.ticketFormatado2024}, 2025=${metricas.ticketFormatado2025}`
      );
    });
  ...
  atualizarTabelaPerformanceDetalhada(html, dados) {
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      ...
      if (html.match(linhaPattern)) {
        ...
        console.log(
          `  ✅ Performance ${mes}: ${metricas.pedidos2025} pedidos, ${metricas.receitaFormatada2025}`
        );
      }
    });
  ...
  atualizarAnaliseROI(html, dados) {
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      ...
      if (html.match(linhaROIPattern)) {
        ...
        console.log(
          `  ✅ ROI ${mes}: ROAS ${metricas.roas}, Performance ${metricas.performanceROI}`
        );
      }
      ...
      if (html.match(barraPattern)) {
        ...
        console.log(
          `  ✅ Barra ROAS ${mes}: ${metricas.roas} (${roasPercent}%)`
        );
      }
    });
  ...
  atualizarIndicadoresEficiencia(html, dados) {
    ...
    console.log(
      `  ✅ Eficiência atualizada: ${pedidosMes2025} pedidos/mês, ROAS ${roasMedio2025}x`
    );
    return html;
  }
  ...
  atualizarKPIsPrincipais(html, dados, config) {
    ...
    html = html.replaceAll(
      "(+98%)",
      `(${dados.formatados.crescimentoPedidos})`
    );
    html = html.replaceAll(
      "(+148%)",
      `(${dados.formatados.crescimentoReceita})`
    );
  ...
  atualizarTextosEConclusoes(html, dados, config) {
    // Destaque
    const destaque = `${
      config.relatorio?.plataforma || "Google Ads"
    } apresentou crescimento excepcional em 2025, com ROAS médio de ${
      dados.totais.roasGeral
    }x e crescimento de ${dados.formatados.crescimentoPedidos.replace(
      "+",
      ""
    )} nos pedidos vs 2024.`;
  ...
  logDadosPorSecao(dados) {
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      console.log(
        `  ${mes}: 2024=${metricas.ticketFormatado2024}, 2025=${metricas.ticketFormatado2025} (${metricas.crescTicket})`
      );
    });
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      console.log(
        `  ${mes}: Pedidos 2025=${metricas.pedidos2025}, Receita 2025=${metricas.receitaFormatada2025}`
      );
    });
    ...
    Object.entries(dados.metricasPorMes).forEach(([mes, metricas]) => {
      console.log(
        `  ${mes}: ROAS ${metricas.roas}, ROI ${metricas.roi}, ${metricas.performanceROI}`
      );
    });
  }
  ...
  calcularDadosCorrigidos(dados, investimento) {
    ...
    dados.forEach((item, index) => {
      console.log(
        `  ${index + 1}. Data: ${item.data}, Pedidos: ${
          item.pedidos
        }, Receita: ${this.formatarMoeda(item.valorBruto)}`
      );
    });
    ...
    dados.forEach((item) => {
      ...
      if (ano === 2024) {
        ...
        console.log(
          `    + 2024: Pedidos: ${item.pedidos}, Receita: ${this.formatarMoeda(
            item.valorBruto
          )}`
        );
      } else if (ano === 2025) {
        ...
        console.log(
          `    + 2025: Pedidos: ${item.pedidos}, Receita: ${this.formatarMoeda(
            item.valorBruto
          )}`
        );
      }
    ...
    console.log(
      `  2024: ${total2024.pedidos} pedidos, ${this.formatarMoeda(
        total2024.receita
      )}`
    );
    console.log(
      `  2025: ${total2025.pedidos} pedidos, ${this.formatarMoeda(
        total2025.receita
      )}`
    );

    // Calcular crescimentos CORRETOS
    const crescimentoPedidos =
      total2024.pedidos > 0
        ? `+${(
            ((total2025.pedidos - total2024.pedidos) / total2024.pedidos) *
            100
          ).toFixed(1)}%`
        : "+0.0%";
    ...
    const crescimentoReceita =
      total2024.receita > 0
        ? `+${(
            ((total2025.receita - total2024.receita) / total2024.receita) *
            100
          ).toFixed(1)}%`
        : "+0.0%";
    ...
    const crescimentoTicket =
      ticketMedio2024 > 0
        ? `+${(
            ((ticketMedio2025 - ticketMedio2024) / ticketMedio2024) *
            100
          ).toFixed(1)}%`
        : "+0.0%";
    ...
    const roasGeral =
      total2025.receita > 0
        ? (total2025.receita / investimentoTrimestre).toFixed(1)
        : "0.0";
  ...
  substituirKPIsForcado(html, dados, updates) {
    ...
    patterns1.forEach((pattern) => {
      if (!encontrou1 && pattern.test(html)) {
        html = html.replace(
          pattern,
          `<div class="kpi-value">${dados.totalPedidosFormatado}</div>`
        );
        encontrou1 = true;
    ...
    patterns2.forEach((pattern) => {
      if (!encontrou2 && pattern.test(html)) {
        html = html.replace(
          pattern,
          `<div class="kpi-value">${dados.receitaTotalFormatada}</div>`
        );
        encontrou2 = true;
    ...
    html = html.replace(/class="kpi-value">R\$ [^<]*<\/div>/g, (match) => {
      kpiCount++;
      if (kpiCount === 2) {
        // Segundo KPI com R$ é o ticket médio
  ...
  substituirParagrafosForcado(html, dados, config, updates) {
    ...
    padrõesConclusão.forEach((padrao) => {
      if (!conclusaoAtualizada && padrao.test(html)) {
        ...
        updates.push(
          `Conclusão atualizada: ${dados.investimentoTrimestreFormatado}/trimestre`
        );
      }
    });
    ...
    if (allParagraphs) {
      ...
      allParagraphs.forEach((paragraph, index) => {
        console.log(
          `  Parágrafo ${index + 1}: ${paragraph.substring(0, 100)}...`
        );

        // Verificar se contém textos que precisam ser atualizados
        if (
          paragraph.includes("Google Ads demonstra") ||
          paragraph.includes("Com investimento atual")
        ) {
          console.log(
            `  ⚠️ Parágrafo ${index + 1} precisa ser atualizado manualmente`
          );
        }
      });
    }
  ...
  substituirSectionsForcado(html, dados, updates) {
    ...
    if (allSections) {
      ...
      allSections.forEach((section, index) => {
        ...
        if (
          section.includes("🏁 Conclusão") ||
          section.includes("highlight-box")
        ) {
          ...
          if (section.includes("Google Ads demonstra")) {
            const novaSection = section.replace(
              ...
              `${
                dados.plataforma || "Google Ads"
              } demonstra performance excepcional e é o canal com melhor ROI da operação.`
            );
  ...
  substituirGraficosForcado(html, dados, updates) {
    ...
    html = html.replace(/<div class="bar-value">\d+<\/div>/g, (match) => {
      ...
      if (graficoIndex < graficosData.length) {
        const valor = isBar2024
          ? graficosData[graficoIndex].val2024
          : graficosData[graficoIndex].val2025;
        barCount++;
  ...
  verificarSubstituicoes(html, dados) {
    ...
    const verificacoes = [
      {
        elemento: "Total Pedidos",
        busca: dados.totalPedidosFormatado,
        encontrado: html.includes(dados.totalPedidosFormatado),
      },
      {
        elemento: "Receita Total",
        busca: dados.receitaTotalFormatada,
        encontrado: html.includes(dados.receitaTotalFormatada),
      },
      {
        elemento: "Crescimento Pedidos",
        busca: dados.crescimentoPedidos,
        encontrado: html.includes(dados.crescimentoPedidos),
      },
      {
        elemento: "ROAS",
        busca: `${dados.roasGeral}x`,
        encontrado: html.includes(`${dados.roasGeral}x`),
      },
    ];
  ...
  calcularMetricasPorMes(dadosOrganizados, investimento) {
    ...
    meses.forEach((mes, index) => {
      const nome = nomesMeses[index];
      const dados2024 = dadosOrganizados[2024]?.[mes] || {
        pedidos: 0,
        receita: 0,
      };
      const dados2025 = dadosOrganizados[2025]?.[mes] || {
        pedidos: 0,
        receita: 0,
      };
  ...
  atualizarTabelaPerformance(html, porMes, updates) {
    ...
    Object.entries(porMes).forEach(([mes, dados]) => {
      ...
      if (linhaPattern.test(html)) {
        html = html.replace(linhaPattern, novaLinha);
        updates.push(
          `Tabela Performance ${mes}: ${dados.pedidos2025} pedidos, ${dados.receita2025}`
        );
      }
    });
  ...
  atualizarGraficos(html, graficos, updates) {
    ...
    Object.entries(graficos).forEach(([mes, dados]) => {
      ...
      if (barra2024Pattern.test(html)) {
        ...
        updates.push(
          `Gráfico ${mes} 2024: ${dados.pedidos2024} (${dados.altura2024})`
        );
      }
      ...
      if (barra2025Pattern.test(html)) {
        ...
        updates.push(
          `Gráfico ${mes} 2025: ${dados.pedidos2025} (${dados.altura2025})`
        );
      }
    });
  ...
  atualizarResumoFinal(html, metricas, updates) {
    ...
    const roiTotalPercent = (
      ((metricas.totais.receita2025 - 9000 * 3) / (9000 * 3)) *
      100
    ).toFixed(0);
    ...
    progressBars.forEach((bar) => {
      const pattern = new RegExp(
        `<div class="metric-name">[^<]*${
          bar.name.split(" ")[0]
        }[^<]*</div>[\s\S]*?<div class="metric-value">[^<]*</div>`
      );
  ...
  generateHTML() {
    ...
    try {
      ...
      if (!config) {
        log(
          "❌ Falha ao carregar configuração, abortando geração HTML",
          "error"
        );
        return null;
      }
      ...
      if (!css) {
        log(
          "⚠️ CSS não carregado, continuando sem estilos incorporados",
          "warning"
        );
      }
  ...
  async generatePDF(htmlPath) {
    ...
    try {
      ...
      const pdfPath = path.join(
        this.outputDir,
        `relatorio-google-ads-${timestamp}.pdf`
      );
      ...
      // Na função generatePDF, substitua as opções do PDF:
      await page.pdf({
        path: pdfPath,
        format: "A4",
        printBackground: true,
        margin: {
          top: "10mm", // REDUZIDO de 20mm
          right: "10mm", // REDUZIDO de 15mm
          bottom: "10mm", // REDUZIDO de 20mm
          left: "10mm", // REDUZIDO de 15mm
        },
        preferCSSPageSize: true,
        scale: 0.9, // ADICIONADO: Reduz ligeiramente o tamanho para caber mais conteúdo
      });
      ...
      log(
        `✅ PDF gerado em ${pdfTime}s (${pdfSize} KB): ${pdfPath}`,
        "success"
      );
    ...
    try {
      ...
      meses.forEach((mes, index) => {
        ...
        graficos[nomeMes] = {
          ...
          altura2024: `${Math.max(
            20,
            (dados2024.pedidos / maxPedidos) * 100
          ).toFixed(0)}%`,
          altura2025: `${Math.max(
            20,
            (dados2025.pedidos / maxPedidos) * 100
          ).toFixed(0)}%`,
        };
        ...
        tabelas[nomeMes] = {
          ...
          receita2024: this.formatarMoeda(dados2024.receita),
          receita2025: this.formatarMoeda(dados2025.receita),
          crescPedidos: this.calcularCrescimento(
            dados2024.pedidos,
            dados2025.pedidos
          ),
          crescReceita: this.calcularCrescimento(
            dados2024.receita,
            dados2025.receita
          ),
        };
        ...
        // ROI
        const roiCalc =
          ((dados2025.receita - investimento) / investimento) * 100;
      ...
      const resultado = {
        ...
        destaque: `${
          config.relatorio.plataforma
        } apresentou crescimento excepcional em 2025, com ROAS médio de ${roasResultado.toFixed(
          1
        )}x e crescimento de ${crescPedidos} nos pedidos vs 2024.`,
        conclusao: `Com investimento atual de ${this.formatarMoeda(
          investimentoTrimestre
        )}/trimestre, gerou ${this.formatarMoeda(
          total2025.receita
        )} em receita, resultando em um ROAS médio de ${roasResultado.toFixed(
          1
        )}x - muito acima da média de mercado.`,
      };
